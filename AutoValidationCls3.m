classdef AutoValidationCls3
% AutoValidationCls2('TC6_.txt', 0, 0);
% AutoValidationCls3('TC6_.txt', 0, 0, 1)  % MATファイル比較
...
% C12 OK:一致、NG：不一致
 ...
 %      └─ ...
    ...
    methods
        function obj = AutoValidationCls3(filename, summaryMode, mode, compareSourceMode)
            tic;
            clc;
            warning off;
            ...
            if nargin < 4
             ...
             obj.compareSourceMode = compareSourceMode;
    ...
    % try
        ...
        for k = 1:length(validIdx)% numTests
            ...
            try
                ...
                if obj.compareSourceMode == 0
                    % save([TCName, '_', num2str(i), '.mat'], 'TEST', '-v7.3');
                    save(['ToFileName_', num2str(suffix), '.mat'], 'TEST', '-v7.3');
                end
...
if obj.compareSourceMode == 0
    compareTarget = simout;
else
    matFile = ['ToFileName_', num2str(suffix), '.mat'];
    if ~exist(matFile, 'file')
        error('%s が見つかりません。', matFile);
    end
    loadedData = load(matFile);
    simoutDummy.logsout = ...
        obj.structToLogsout(loadedData.TEST);
    compareTarget = simoutDummy;
end

[tmp_sigdata, tmp_rawdata, tmp_editdata, tmp_judgement] = ...
    obj.evaluateSimulationOutput( ...
        compareTarget, targetSignals, arg_config.okng_mode);
                % [tmp_sigdata, tmp_rawdata, tmp_editdata, tmp_judgement] = ...
...
%% 
function logsoutFromMat = structToLogsout(obj, TEST)

    logsoutFromMat = Simulink.SimulationData.Dataset;
    ...
    for i = 1:numel(sigNames)
        ...
        logsoutFromMat = logsoutFromMat.addElement(elem);
    end
end
